...
def option_model_nllh(params, D, structure, meta_learning=True):
	...
	p_policies = np.array([1-eps-prior, prior, eps])
	...
	for t in range(D.shape[0]):	
		...
		if stage == 1:
			...
			for i in range(PTS_2.shape[0]):
				TS_2s[i,state,a_2-1] += alpha_2 * (r_2 - TS_2s[i,state,a_2-1]) * PTS_2[i,c_2] 
...
def option_model(num_subject, alpha_1, alpha_2, beta_1, beta_2, concentration_1, concentration_2, epsilon, prior, experiment, structure, meta_learning=True, debug=False):
	...
	for sub in range(num_subject):
		...
		PTS_2 = np.ones((nTS_2,nC_2))
		eps = 0.002
		p_policies = np.array([1-eps-prior, prior, eps]) # compression over stage 1, compression over stage 2, full hierarchical
