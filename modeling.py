...
def abstraction_model_nllh(params, D, structure, meta_learning=1, fix_forget=False):
	'''
	...
	[alpha_2, beta, forget, concentration_2, epsilon, prior_h, prior_w] = params
	beta_2 = beta
	beta_policies = 5
	concentration_2 = 10**concentration_2
	forget = 1 if fix_forget else forget
	...
	for t in range(D.shape[0]):	# loop over all trials
		...
		if stage == 1: # skip stage1
			...
			if meta_learning == 1:
				...
				# forget
				likelihoods_compressed_1 = (1 - forget) * likelihoods_compressed_1 + forget
				likelihoods_compressed_2 = (1 - forget) * likelihoods_compressed_2 + forget
				likelihoods_full = (1 - forget) * likelihoods_full + forget
...
def abstraction_model(num_subject, params, experiment, structure, meta_learning=1, fix_forget=False):
	'''
...
def parallel_worker(args):
	...
	'''
	fit_model_name, data, structure, i, param_bounds, meta_learning, fix_forget = args
	best_params, best_llh = optimize(globals()[fit_model_name+'_nllh'], param_bounds, data, structure, meta_learning, fix_forget)
...
def parallel_simulator(args):
	...
	'''
	this_model, i, niters_sim, params, exp, structure, meta_learning, fix_forget = args
	this_data = globals()[this_model](niters_sim, params, exp, structure, meta_learning, fix_forget)
	return i, this_data