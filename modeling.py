...
def option_model_nllh(params, D, structure, meta_learning=True):
	...
	for t in range(D.shape[0]):	
		...
		if stage == 1:
			...
			TS_2s[:,1-state,a_2-1] -= alpha_cf * RPE * PTS_2[:,c_2]
			TS_2s[:,state,a_2-1] -= alpha_cf * RPE * PTS_2[:,c_2_alt]
			TS_2s[:,1-state,a_2-1] -= alpha_cf * RPE * PTS_2[:,c_2_alt]
			...
			for trial in range(num_trial_12):	
				...
				if correct_1 == 1: # Otherwise, jump to the next trial to mimic real experimental setting
					while correct_2 == 0 and counter_2 < 10:
						...
						TS_2s[TS_2,1-state,a_2-1] -= alpha_cf * RPE
						TS_2s[TS_2_alt,:,a_2-1] -= alpha_cf * RPE
